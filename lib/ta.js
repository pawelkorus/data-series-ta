...
function MACD(a, b, c) {
	var constantValueIterator = function() {
		return {
			data: undefined,
			...
			setConstant: function(v) {
				this.data = v;	
			}
		}
	}
	
	var constantIterator = constantValueIterator();

	var EMAa = functionOperator(constantIterator, EMA(a));
	var EMAb = functionOperator(constantIterator, EMA(b));
	var EMAc = functionOperator(constantIterator, EMA(c));
	
	var f = function(v) {
		if(v === undefined) return undefined;

		constantIterator.setConstant(v);

		return [EMAa.next() - EMAb.next(), EMAc.next()];
	}
