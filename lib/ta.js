...
var valueProvider = require('data-series').valueProvider;
...
function MACD(a, b, c) {
	var macdLineIt = valueProvider();
	var signalLineIt = valueProvider();
...
function TSI(r, s) {
	var diffProvider = valueProvider();
	var EMAr = functionOperator(diffProvider, EMA(r));
	var EMAs = functionOperator(EMAr, EMA(s));
	
	var absDiffProvider = valueProvider();
	var EMArAbs = functionOperator(absDiffProvider, EMA(r));
	var EMAsAbs = functionOperator(EMArAbs, EMA(s));
	...
	return function(v) {
		...
		if(prevValue === undefined) {
			...
			var diff = prevValue - v;
			diffProvider.setValue(diff);
			absDiffProvider.setValue(Math.abs(diff));

			prevValue = v;

			return 100 * EMAs.next()/EMAsAbs.next();
		}
	}
}
